...
namespace OS_lab3
{
    class win32API
    {
        ...
        /// <summary>
        /// Получает имя модуля по его дескриптору
        /// </summary>
        /// <param name="hProcess">Дескриптор процесса</param>
        ...
        public static void pauseProcess(IntPtr handle, UInt32 maxPauseMilliseconds, bool isPaused)
        {
            if (!isPaused)
            {
                ...
                win32APIOld.CloseHandle(handle);
            }
        }
        ...
        public static IntPtr getProcessHandle(String nameSearchProcess)
        {
            ...
            UInt32 currentProcessId = win32APIOld.GetCurrentProcessId();
            ...
            for (int i = 0; i < processIds.Length; i++)
            {
                ...
                tmp = win32APIOld.OpenProcess((int)win32APIOld.ProcessAccessFlags.All,
                    false,
                    (int)processIds[i]);

                //Получаем имя процесса
                tmpStr = getProcessName(tmp);
                if (tmpStr.Equals(nameSearchProcess))
                {
