...
namespace Lab3
{
    class Utils
    {
        ...
        public static long GetNumMAC()
        {
            return Convert.ToInt64(GetMAC(), 16);
        }

        public static void Starter(bool mode, bool protocol, long node, ushort socket, MainForm form)
        {
            ...
            if (mode == true)
            {
                ...
                if (protocol == true)
                {
                    client = new IPXClient(node, socket);
                }
                else
                {
                    client = new SPXClient(node, socket);
                }
            ...
            else
            {
                ...
                if (protocol == true)
                {
                    server = new IPXServer(node, socket);
                }
                else
                {
                    server = new SPXServer(node, socket);
                }
