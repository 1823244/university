...
namespace Pentomino
{
    public sealed partial class MainPage : Page
    {
        ...
        private void InitializeFigure(Grid grid)
        {
            ...
            Figures figure = gridToFigures[grid];
            figure.inField = false;
            figure.rows = new List<int>();
            gridToFigures[grid] = figure;
        }
        ...
        private void FindOneSolution(object sender, RoutedEventArgs e)
        {
            ...
            foreach (int[] solve in foundedSolution)
            {
                for (int i = 0; i < solve.Length; i++)
                {
                    if (solve[i] > 0)
                    {
                        if (result[0, solve[i]] > i)
                        {
                            result[0, solve[i]] = i;
                        }

                        if (result[1, solve[i]] < i)
                        {
                            result[1, solve[i]] = i;
                        }
                    }
            ...
            Tester.Text += isSolutionFounded.ToString();
        }
    }
