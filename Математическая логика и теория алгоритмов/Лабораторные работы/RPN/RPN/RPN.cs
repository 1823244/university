...
namespace RPNLib
{
    public class ReversePolishNotation
    {
        ...
        private Stack<string> stOperations = new Stack<string>(); // стек для хранения операций;
        ...
        // возвращает массив строк с именами идентификаторов;
        public string[] GetIdentifiers()
        {
            string[] identifiersString = new string[identifiers.Count];
            int i = 0;
            identifiers.ForEach(delegate(String identifier)
            {
                identifiersString[i++] = identifier;
            });
        ...
        // устанавливает значения для идентификаторов, посредством получения массива логического типа;
        public bool SetValues(bool[] values)
        {
            if(values.Length == identifiers.Count)
            {
                neededIDs.Clear();
                int i = 0;
                foreach (string ID in identifiers)
        ...
        // возвращает количество уникальных идентификаторов;
        ...
        public string Result()
        {
            ...
            foreach (string element in expression)
            {
                ...
                try
                {
                    ...
                    {
                        //Console.WriteLine(stOperations.Count);
                    ...
                    else if (element == ">") // логическое следование;
                    {
                        b1 = Convert.ToBoolean(stOperations.Pop());
                        b2 = Convert.ToBoolean(stOperations.Pop());
                        stOperations.Push((b1 || !b2) ? "true" : "false");
                    }
                    else if (element == "=") // логическое равенство;
                    {
                        b1 = Convert.ToBoolean(stOperations.Pop());
                        b2 = Convert.ToBoolean(stOperations.Pop());
                        stOperations.Push(((!b1 || b2) && (b1 || !b2)) ? "true" : "false");
                    }
                }
