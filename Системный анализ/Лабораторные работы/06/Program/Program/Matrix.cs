...
namespace Matrix
{
	public class Matrix
	{
		...
		/// <summary> умножить матрицу на число </summary>
		public static Matrix operator *(Matrix M, double c)
		{
			Matrix result = new Matrix(M._matr);
			for (int i = 0; i < M.m; i++) for (int j = 0; j < M.n; j++) result[i, j] *= c;
			return result;
		}
		...
		/// <summary> поделить матрицу на число </summary>
		public static Matrix operator /(Matrix M, double c)
		{
			Matrix result = new Matrix(M._matr);
			for (int i = 0; i < M.m; i++) for (int j = 0; j < M.n; j++) result[i, j] /= c;
